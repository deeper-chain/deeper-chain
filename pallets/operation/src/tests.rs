...
const BALANCE_TRANSFER: &<Test as frame_system::Config>::RuntimeCall =
    &<Test as frame_system::Config>::RuntimeCall::Balances(pallet_balances::Call::transfer {
        dest: 1,
        value: 10,
    });
...
fn pause_call_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Operation::paused_calls((b"Balances".to_vec(), b"transfer".to_vec())),
            None
        );
        assert_ok!(Operation::pause_call(
            RuntimeOrigin::signed(1),
            b"Balances".to_vec(),
            b"transfer".to_vec()
        ));
        System::assert_last_event(RuntimeEvent::Operation(crate::Event::Paused(
            "Balances".to_string(),
            "transfer".to_string(),
            1,
        )));
...
fn unpause_transaction_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Operation::unpause_call(
            RuntimeOrigin::signed(1),
            b"Balances".to_vec(),
            b"transfer".to_vec()
        ));
        System::assert_last_event(RuntimeEvent::Operation(crate::Event::Unpaused(
            "Balances".to_string(),
            "transfer".to_string(),
        )));
        assert_eq!(
...
fn paused_transaction_filter_work() {
    new_test_ext().execute_with(|| {
        assert!(!PausedCallFilter::<Test>::contains(BALANCE_TRANSFER));
        assert_ok!(Operation::pause_call(
            RuntimeOrigin::signed(1),
            b"Balances".to_vec(),
            b"transfer".to_vec()
