...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn vote(
            ...
            match new_deposit.cmp(&old_deposit) {
                Ordering::Greater => {
                    ...
                    CurrencyOf::<T>::reserve(&who, to_reserve)
                ...
                Ordering::Less => {
                    ...
                    let _remainder = CurrencyOf::<T>::unreserve(&who, to_unreserve);
                    debug_assert!(_remainder.is_zero());
                }
            };

            // Amount to be locked up.
            let locked_stake = value.min(CurrencyOf::<T>::free_balance(&who));
        ...
        pub fn submit_candidacy(
            ...
            CurrencyOf::<T>::reserve(&who, T::CandidacyBond::get())
        ...
        fn build(&self) {
            ...
            let members = self
                ...
                .map(|(ref member, ref stake)| {
                    ...
                    assert!(
                        CurrencyOf::<T>::free_balance(member) >= *stake,
...
impl<T: Config> Pallet<T> {
    ...
    fn remove_and_replace_member(who: &T::AccountId, slash: bool) -> Result<bool, DispatchError> {
        ...
        let maybe_replacement = <Members<T>>::try_mutate::<_, Error<T>, _>(|members| {
            ...
            // slash or unreserve
            if slash {
                let (imbalance, _remainder) = CurrencyOf::<T>::slash_reserved(who, removed.deposit);
                debug_assert!(_remainder.is_zero());
                ...
                CurrencyOf::<T>::unreserve(who, removed.deposit);
            }
    ...
    fn do_remove_voter(who: &T::AccountId) {
        ...
        CurrencyOf::<T>::remove_lock(T::PalletId::get(), who);
    ...
    fn do_phragmen() -> Weight {
        ...
        let total_issuance = CurrencyOf::<T>::total_issuance();
                ...
                .map(
                    |ElectionResult::<T::AccountId, Perbill> {
                        ...
                        candidates_and_deposit.iter().for_each(|(c, d)| {
                            ...
                            {
                                let (imbalance, _) = CurrencyOf::<T>::slash_reserved(c, *d);
                                T::LoserCandidate::on_unbalanced(imbalance);
...
mod tests {
    ...
    fn member_candidacy_submission_should_not_work() {
        ...
        ExtBuilder::default().build_and_execute(|| {
            ...
            assert_noop!(
                submit_candidacy(RuntimeOrigin::signed(5)),
    ...
    fn cannot_vote_for_no_candidate() {
        ExtBuilder::default().build_and_execute(|| {
            assert_noop!(
                vote(RuntimeOrigin::signed(2), vec![], 20),
                Error::<Test>::NoVotes
            );
        });
    }
    ...
    fn cannot_vote_for_less_than_ed() {
        ExtBuilder::default().build_and_execute(|| {
            ...
            assert_noop!(
                vote(RuntimeOrigin::signed(2), vec![4], 1),
    ...
    fn remove_members_triggers_election() {
        ExtBuilder::default().build_and_execute(|| {
            ...
            assert_ok!(Elections::remove_member(
                RuntimeOrigin::root(),
                4,
                true,
                true
