...
impl<T: Trait> Module<T> {
	...
	fn do_phragmen<Accuracy: PerThing>() -> Option<PrimitiveElectionResult<T::AccountId, Accuracy>> {
		...
		for (validator, _) in <Validators<T>>::iter() {
			// append self vote
			let self_vote = (validator.clone(), Self::slashable_balance_of_vote_weight(&validator)/2 + Self::delegated_credit_score_of_vote_weight(&validator) / 2, vec![validator.clone()]);
		...
		all_nominators.extend(nominator_votes.map(|(n, ns)| {
			let s = Self::slashable_balance_of_vote_weight(&n)/2 + Self::delegated_credit_score_of_vote_weight(&n) / 2;
			(n, s, ns)
		}));
