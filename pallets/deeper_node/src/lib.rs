...
#[derive(Decode, Encode, Default, Clone, Eq, PartialEq)]

pub struct Node<AccountId, BlockNumber> {
    pub account_id: AccountId,
    pub ipv4: IpV4, // IP will not be exposed in future version
    pub country: CountryRegion,
...
pub mod pallet {
    ...
    pub(super) type RegionMapInit<T:Config> = StorageValue<_, bool, ValueQuery>;

    #[pallet::storage]
	#[pallet::getter(fn get_region_code)]
	pub type RegionMap<T: Config> = StorageMap<_, Blake2_128Concat, CountryRegion, CountryRegion, ValueQuery>;

    #[pallet::storage]
	#[pallet::getter(fn get_device_info)]
	pub type DeviceInfo<T: Config> = StorageMap<_, Blake2_128Concat, T::AccountId, Node<T::AccountId, T::BlockNumber>, ValueQuery>;

    #[pallet::storage]
	#[pallet::getter(fn get_servers_by_country)]
	pub type ServersByCountry<T: Config> = StorageMap<_, Blake2_128Concat, CountryRegion, Vec<T::AccountId>, ValueQuery>;

    #[pallet::storage]
	#[pallet::getter(fn get_servers_by_region)]
	pub type ServersByRegion<T: Config> = StorageMap<_, Blake2_128Concat, CountryRegion, Vec<T::AccountId>, ValueQuery>;
	...
	impl<T:Config> Pallet<T> {
        ...
        pub fn register_device(origin: OriginFor<T>, ip: IpV4, country: CountryRegion) -> DispatchResultWithPostInfo {
            let sender = ensure_signed(origin)?;
            ensure!(<RegionMapInit<T>>::get() == true, Error::<T>::InvalidRegionMap);
            ensure!(<RegionMap<T>>::contains_key(&country), Error::<T>::InvalidCode);
        ...
        pub fn unregister_device(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
         ...
         #[pallet::weight(10_000)]
        pub fn register_server(origin: OriginFor<T>, duration: Duration) -> DispatchResultWithPostInfo {
            let sender = ensure_signed(origin)?;
         ...
         #[pallet::weight(10_000)]
        pub fn update_server(origin: OriginFor<T>, duration: Duration) -> DispatchResultWithPostInfo {
            let sender = ensure_signed(origin)?;
            ensure!(<DeviceInfo<T>>::contains_key(&sender),
                    Error::<T>::DeviceNotRegister);
            ensure!(duration <= T::MaxDurationDays::get(), Error::<T>::DurationOverflow);
...
impl<T: Config> Module<T> {
    ...
    fn try_remove_server(sender: &T::AccountId) -> DispatchResultWithPostInfo {
    ...
    fn try_add_server(sender: &T::AccountId, duration: T::BlockNumber) -> DispatchResultWithPostInfo {
        let mut node = <DeviceInfo<T>>::get(&sender);
        let first_region = <RegionMap<T>>::get(&node.country);
        let sec_region = <RegionMap<T>>::get(&first_region);
