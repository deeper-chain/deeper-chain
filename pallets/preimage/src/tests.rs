...
pub fn make_bounded_values() -> (Bounded<Vec<u8>>, Bounded<Vec<u8>>, Bounded<Vec<u8>>) {
    ...
    let data = vec![1, 2, 3];
    let hash: H256 = blake2_256(&data[..]).into();
    let legacy = Bounded::<Vec<u8>>::Legacy {
        hash,
...
fn query_and_store_preimage_workflow() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Preimage::peek::<Vec<u8>>(&bound).unwrap(),
            (data.clone(), Some(len))
        );
        ...
        assert_eq!(
            Preimage::realize::<Vec<u8>>(&bound).unwrap(),
            (data.clone(), Some(len))
        );
        assert!(matches!(
            StatusFor::<Test>::get(&hash).unwrap(),
...
fn query_preimage_request_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            <Preimage as QueryPreimage>::fetch(&hash, None),
            DispatchError::Unavailable
        );
...
fn store_preimage_basic_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            <Preimage as QueryPreimage>::peek(&bound),
            DispatchError::Unavailable
        );
...
fn store_preimage_note_too_large_errors() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            <Preimage as StorePreimage>::note(data.into()),
...
fn store_preimage_bound_too_large_errors() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            <Preimage as StorePreimage>::bound(data.clone()),
            DispatchError::Exhausted
        );
