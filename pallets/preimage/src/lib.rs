...
impl<T: Config> Pallet<T> {
    ...
    fn note_bytes(
        ...
        let status = match (StatusFor::<T>::get(hash), maybe_depositor) {
            ...
            (Some(RequestStatus::Unrequested { len, deposit }), None) => RequestStatus::Requested {
                deposit: Some(deposit),
                count: 1,
                len: Some(len),
            },
            (None, None) => RequestStatus::Requested {
                count: 1,
                len: Some(len),
                deposit: None,
            },
            (None, Some(depositor)) => {
                ...
                RequestStatus::Unrequested {
                    deposit: (depositor.clone(), deposit),
                    len,
                }
            }
        };
