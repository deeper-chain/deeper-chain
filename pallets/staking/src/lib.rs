...
pub mod pallet {
    ...
    /// Number of blocks per era.
        ...
        pub type ValidatorWhiteList = StorageValue<_, Vec<T::AccountId>, ValueQuery>;

        /// Map from all locked "stash" accounts to the controller account.
        #[pallet::storage]
        #[pallet::getter(fn bonded)]
        pub type Bonded<T> = StorageMap<_, Twox64Concat, T::AccountId, T::AccountId>;
	...
	pub type EraElectionStatus<T: Config> = StorageValue<_, ElectionStatus<T::BlockNumber>, ValueQuery>;
	...
	#[pallet::getter(fn remainder_mining_reward)]
        pub type RemainderMiningReward = StorageValue<_,u128>;

        /// validator -> ValidatorData
        #[pallet::storage]
	#[pallet::getter(fn candidate_validators)]
	pub type CandidateValidators<T: Config> =
		StorageMap<_, blake2_128_concat, T::AccountId, ValidatorData<T::AccountId>, ValueQuery>;
        ...
        pub type CandidateValidators<T: Config> =
            ...
            pub type Delegators<T: Config> =
                StorageMap<_, blake2_128_concat, T::AccountId, DelegatorData<T::AccountId>, ValueQuery>;
       
        /// active delegator count
        #[pallet::storage]
	#[pallet::getter(fn active_delegator_count)]
    pub type ActiveDelegatorCount = StorageValue<_,u32>;

    #[pallet::storage]
	#[pallet::getter(fn delegator_count)]
    pub type DelegatorCount = StorageValue<_,u32>;

        /// delegators key prefix
        #[pallet::storage]
        #[pallet::getter(fn delegators_key_prefix)]
        pub type DelegatorsKeyPrefix = StorageValue<_,Vec<u8>>;


        /// delegators last key
        #[pallet::storage]
        #[pallet::getter(fn delegators_last_key)]
        pub type DelegatorsLastKey = StorageValue<_,Vec<u8>>;

        #[pallet::storage]
	#[pallet::getter(fn delegator_payouts_per_block)]
     type DelegatorPayoutsPerBlock = StorageValue<_,u32>;

        /// EraIndex -> validators
        #[pallet::storage]
            #[pallet::getter(fn eras_validators)]
            type ErasValidators<T: Config> =
                StorageMap<_, blake2_128_concat, EraIndex, Vec<T::AccountId>, ValueQuery>;
       
        /// reward of delegator
        #[pallet::storage]
            #[pallet::getter(fn reward)]
            pub type Reward<T: Config> =
                StorageMap<_, blake2_128_concat, T::AccountId, RewardData<BalanceOf<T>>>;
...
impl<T: Config> Pallet<T> { 
    ...
    pub fn bond(origin: OriginFor<T>,
    ...
    fn withdraw_unbonded(origin: OriginFor<T>, num_slashing_spans: u32) -> DispatchResultWithPostInfo {
        ensure!(Self::era_election_status().is_closed(), Error::<T>::CallNotAllowed);
        let controller = ensure_signed(origin)?;
        let mut ledger = Self::ledger(&controller).ok_or(Error::<T>::NotController)?;
    ...
    pub fn validate(origin: OriginFor<T>, prefs: ValidatorPrefs) {
        ensure!(Self::era_election_status().is_closed(), Error::<T>::CallNotAllowed);
        let controller = ensure_signed(origin)?;
    ...
    fn set_era_validator_reward(origin: OriginFor<T>, #[compact] value: BalanceOf<T>) {
        ensure_root(origin)?;
    ...
    fn set_validator_count(origin: OriginFor<T>, #[compact] new: u32) {
        ensure_root(origin)?;
    ...
    /// # </weight>
    #[pallet::weight(T::WeightInfo::increase_validator_count())]
    fn increase_validator_count(origin: OriginFor<T>, #[compact] additional: u32) {
        ensure_root(origin)?;
    ...
    /// Same as [`set_validator_count`].
    /// # </weight>
    #[pallet::weight(T::WeightInfo::scale_validator_count())]
    fn scale_validator_count(origin: OriginFor<T>, factor: Percent) {
        ensure_root(origin)?;
    ...
    fn force_no_eras(origin: OriginFor<T>) {
        ensure_root(origin)?;
    ...
    fn force_new_era(origin: OriginFor<T>) {
        ensure_root(origin)?;
    ...
    fn set_invulnerables(origin: OriginFor<T>, invulnerables: Vec<T::AccountId>) {
        ensure_root(origin)?;
    ...
    fn set_validator_whitelist(origin: OriginFor<T>, whitelist: Vec<T::AccountId>) {
        ensure_root(origin)?;
    ...
    fn force_unstake(origin: OriginFor<T>, stash: T::AccountId, num_slashing_spans: u32) {
        ensure_root(origin)?;
    ...
    fn force_new_era_always(origin: OriginFor<T>) {
        ensure_root(origin)?;
        ForceEra::put(Forcing::ForceAlways);
    }

    #[pallet::weight(T::WeightInfo::increase_mining_reward(1))]
    fn increase_mining_reward(origin: OriginFor<T>, additional_reward: u128) {
        ensure_root(origin)?;
    ...
    fn cancel_deferred_slash(origin: OriginFor<T>, era: EraIndex, slash_indices: Vec<u32>) {
    ...
    fn rebond(origin: OriginFor<T>, #[compact] value: BalanceOf<T>) -> DispatchResultWithPostInfo {
        ensure!(Self::era_election_status().is_closed(), Error::<T>::CallNotAllowed);
        let controller = ensure_signed(origin)?;
        let ledger = Self::ledger(&controller).ok_or(Error::<T>::NotController)?;
    ...
    fn set_history_depth(origin: OriginFor<T>,
    ...
    fn reap_stash(_origin: OriginFor<T>, stash: T::AccountId, num_slashing_spans: u32) {
...
pub enum Event<T: Config> {
        ...
        DelegatorReward(AccountId, BalanceOf<T>),
        /// The validator  has been rewarded by this amount. \[account_id, amount\]
        ValidatorReward(AccountId, BalanceOf<T>),
    }
