...
pub fn clear_validators_and_delegators<T: Config>() {
    let _ = Validators::<T>::clear(u32::max_value(), None);
    let _ = CandidateValidators::<T>::clear(u32::max_value(), None);
    let _ = Delegators::<T>::clear(u32::max_value(), None);
}

/// Grab a funded user.
pub fn high_funded_user<T: Config>(user: &T::AccountId, balance_factor: u32) {
