...
fn expected_regex() -> Regex {
    Regex::new(r"^deeper-chain (\d+\.\d+\.\d+(?:-.+?)?)-([a-f\d]+|unknown)$").unwrap()
}
...
fn test_regex_matches_properly() {
    ...
    let captures = expected.captures("deeper-chain 2.0.0-da487d19d").unwrap();
    assert_eq!(&captures[1], "2.0.0");
    assert_eq!(&captures[2], "da487d19d");

    let captures = expected
        .captures("deeper-chain 2.0.0-alpha.5-da487d19d")
        .unwrap();
