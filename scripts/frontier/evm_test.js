...
let createContract = (exports.createConstract = async function (api, source, init, value, gas_limit, max_fee_per_gas, max_prio_fee_per_gas, nonce, access_list) {
    const keyring = new Keyring({ type: 'sr25519' });
    const signer = keyring.addFromUri('//Alice', { name: 'Alice default' });
    const txHash = await api.tx.evm
        .create(source, init, value, gas_limit, max_fee_per_gas, max_prio_fee_per_gas, nonce, access_list)
...
let tokenTransfer = (exports.tokenTransfer = async function (api, source, target, input, value, gas_limit, max_fee_per_gas, max_priority_fee_per_gas, nonce, access_list) {
    const keyring = new Keyring({ type: 'sr25519' });
    const signer = keyring.addFromUri('//Alice', { name: 'Alice default' });
    const txHash = await api.tx.evm
        .call(source, target, input, value, gas_limit, max_fee_per_gas, max_priority_fee_per_gas, nonce, access_list)
        .signAndSend(signer, ({ events = [], status }) => {
            if (status.isFinalized) {
                events.forEach(({ phase, event: { data, method, section } }) => {
                    if (method == "ExtrinsicFailed")
...
async function test() {
    ...
    await createContract(api, alice, erc20_bytecode, 0, 4294967295, 1000, null, null, null);
    ...
    await tokenTransfer(api, alice, smart_contract_address, input, 0, 4294967295, 1000, null, null, null);
