    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let signed = self.keyring.sign(
            CheckedExtrinsic {
                signed: Some((
                    sender,
                    signed_extra(0, deeper_chain_runtime::ExistentialDeposit::get() + 1),
                )),
                function: match self.content.block_type {
                    BlockType::RandomTransfersKeepAlive => {
                        Call::Balances(BalancesCall::transfer_keep_alive(
                            pallet_indices::address::Address::Id(receiver),
                            deeper_chain_runtime::ExistentialDeposit::get() + 1,
                        ))
                    }
                    BlockType::RandomTransfersReaping => {
                        Call::Balances(BalancesCall::transfer(
                            ...
                            100 * DOLLARS - (deeper_chain_runtime::ExistentialDeposit::get() - 1),
                        ))
                    }
...
impl BenchKeyring {
    ...
    pub fn generate_genesis(&self) -> deeper_chain_runtime::GenesisConfig {
        crate::genesis::config_endowed(
            false,
            Some(deeper_chain_runtime::wasm_binary_unwrap()),
